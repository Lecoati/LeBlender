    ...
    public class Helper
    {
        ...
        public static int GetCacheExpiration(string leBlenderEditorAlias)
        {
            ...
            try
            {
                ...
                if (editor != null && editor.Config.ContainsKey("expiration") && editor.Config["expiration"] != null)
                {
        ...
        internal static IEnumerable<GridEditor> GetLeBlenderGridEditors(bool onlyLeBlenderEditor)
        {
            List<GridEditor> GetResult()
            {
                ...
                if (!System.IO.File.Exists(gridConfig)) return editors;
                ...
                try
                {
                    ...
                    {
                        editors = editors.Where(r => r.View.Equals("/App_Plugins/LeBlender/core/LeBlendereditor.html", StringComparison.InvariantCultureIgnoreCase) || r.View.Equals("/App_Plugins/LeBlender/editors/leblendereditor/LeBlendereditor.html", StringComparison.InvariantCultureIgnoreCase)).ToList();
                    }
                }
                catch (Exception ex)
                {
            ...
            if (result != null && onlyLeBlenderEditor) return result;
        ...
        internal static Type GetLeBlenderController(string editorAlias)
        {
            ...
            if (!controllersList.Any()) return null;

            var controllersFilter = controllersList.Where(t => t.Name.Equals(editorAlias + "Controller", StringComparison.InvariantCultureIgnoreCase));
            var controllersFilterList = controllersFilter as Type[] ?? controllersFilter.ToArray();

            var result = controllersFilterList.Any() ? controllersFilterList.First() : null;
            return result;
        }
        ...
        internal static PublishedContentType GetTargetContentType()
        {
            ...
            if (!string.IsNullOrEmpty(HttpContext.Current.Request["doctype"]))
            {
                return PublishedContentType.Get(PublishedItemType.Content, HttpContext.Current.Request["doctype"]);
            }

            var contenId = int.Parse(HttpContext.Current.Request["id"]);
